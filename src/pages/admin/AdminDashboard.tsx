      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8 mb-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-lg font-black text-gray-900">Thống kê học tập (Quiz)</h2>
              <p className="text-gray-500 font-medium mt-1">Điểm trung bình theo ngày (7 ngày gần nhất)</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
              <div className="bg-gray-50 rounded-2xl px-4 py-3 border border-gray-100">
                <div className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Total attempts</div>
                <div className="text-sm font-black text-gray-800 mt-1">{Number(stats?.learning?.totalAttempts ?? 0).toLocaleString()}</div>
              </div>
              <div className="bg-gray-50 rounded-2xl px-4 py-3 border border-gray-100">
                <div className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Avg % overall</div>
                <div className="text-sm font-black text-gray-800 mt-1">{Number(stats?.learning?.avgPercentageOverall ?? 0).toFixed(2)}%</div>
              </div>
            </div>
          </div>
              ...
              <div className="mt-4 grid grid-cols-2 md:grid-cols-7 gap-2">
                {learningSeries.map((d) => (
                  <div key={d.date} className="bg-gray-50 rounded-2xl px-3 py-2 border border-gray-100">
                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{String(d.date).slice(5)}</div>
                    <div className="text-xs font-black text-gray-800 mt-1">{Number(d.avgPercentage ?? 0).toFixed(2)}%</div>
        ...
        <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
          <p className="text-sm font-bold text-gray-600">Bạn có thể quản trị Users/Courses/Payments/Reviews/Categories ở menu bên trái.</p>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-gray-50 rounded-2xl px-4 py-3 border border-gray-100">
              <div className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Payments completed</div>
              <div className="text-sm font-black text-gray-800 mt-1">{Number(stats?.completedPayments ?? 0).toLocaleString()}</div>
            </div>
            <div className="bg-gray-50 rounded-2xl px-4 py-3 border border-gray-100">
              <div className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Payments failed</div>
              <div className="text-sm font-black text-gray-800 mt-1">{Number(stats?.failedPayments ?? 0).toLocaleString()}</div>
            </div>
            <div className="bg-gray-50 rounded-2xl px-4 py-3 border border-gray-100">
              <div className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Avg rating</div>
              <div className="text-sm font-black text-gray-800 mt-1">{Number(stats?.avgRating ?? 0).toFixed(2)}</div>
            </div>
          </div>
        </div>
      </div>
